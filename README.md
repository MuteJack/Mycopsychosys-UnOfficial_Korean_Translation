...
> Note: 원본 게임은 한글 폰트를 지원하지 않으므로 폰트 깨짐 문제가 발생할 수 있습니다. 해당 사항에 대해서는 7번을 참고해주세요.

1. 게임의 `/game/` 폴더에 해당 레포지터리의 `/game`폴더를 덮어씌워주세요.
2. 버튼 이미지:  `\game\images\images\Buttons` 폴더에서 `/korean`폴더의 이름을 `/russian`으로 이름을 변경해주세요.
3. 기존 `/game/tl` 폴더에서 `/russian`폴더를 삭제하고,`/korean`폴더의 이름을 `/russian`으로 이름을 변경해주세요.
4. 각 .rpy파일들을 열고, `translate korean` 라 작성된 부분을 모두 `translate russian`으로 일괄 변경합니다.
5. 게임을 켜고, 러시아어 `(russian)`을 선택해주세요.
6. `russian`외에도, 게임에서 지원하는 언어면 모두 적용 가능합니다.
7. `options.rpy`에 한글 폰트에 대한 설정이 추가적으로 작성되어있습니다.
...
- 3. `options.rpy`에 한글 폰트에 대한 설정이 추가적으로 작성되어있습니다.  
원하신다면 `options.rpy`파일을 참고/수정하여 나눔고딕이 아닌 원하시는 다른 폰트를 사용하실 수 있습니다.
...
본 레포지터리는 [비공식 번역/기능 패치](https://github.com/MuteJack/Mycopsychosys-UnOfficial-Patch)의 번역 스크립트와 번역된 이미지만을 포함하고 있습니다.
...
- 번역 버전: v1.0.3 (2026.02.02)

## 주의사항

- 해당 번역은 비공식 팬메이드이며, DeltaCat Studio와 관련이 없습니다.
  - 게임 "Mycopsychosys: Remastered"의 모든 저작권은 DeltaCat Studio에 있습니다.
  - 본 번역은 어떠한 보증 없이 제공되며, 무단 사용 및 재배포로 인해 문제에 발생하는 문제에 대해 제작자는 책임지지 않습니다.
  - 본 번역 제작자는 DeltaCat Studio와 그들의 저작물을 존중하며, 원본 게임의 권리를 침해할 의도가 없음을 밝힙니다.
  
- 적용 대상 버전(게임)은 **Mycopsychosys Remastered v2.0 (2026.01.26 Update)**입니다.
  - 게임이 업데이트되었을 경우, 일부 번역이 적용되지 않는 등의 문제가 발생할 수 있습니다.
